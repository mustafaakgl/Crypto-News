...
const en = {
  ...
  home: {
    ...
    ariaStoryFilter: "Filter top stories by coin",
    filterAll: "All",
    filterBitcoin: "Bitcoin News",
    filterEthereum: "Ethereum News",
    noStoriesForFilter: "No recent stories for this coin.",
  ...
  prices: {
    ...
    liveSourceLine: (time: string) => `Live · Binance USDT pairs · 24h change · updated ${time}`,
  ...
  exchangeAnalytics: {
    ...
    subheading:
      "Spot trading volume on centralized (CEX) and decentralized (DEX) exchanges, stablecoin, ETH and BTC flows on tracked exchange wallets, and price gaps between the two kinds of market — no futures/perpetuals, no LLM-generated commentary.",
  ...
  exchangeVolume: {
    ...
    colAllPairsPeriod: (period: string) => `All pairs, ${period}`,
    colAllPairs24h: "All pairs, last 24h",
    colTrackedShare: "Tracked share",
    colStableSwap24h: "Stable ↔ stable",
    colPairsCounted: "Pairs",
    allPairsNote:
      "Every spot pair on each exchange. The period column sums completed UTC days from the collected history; tracked share is how much of it the major pairs in the other views cover. The last-24h columns are a rolling ticker snapshot, shown separately and never added into a period.",
    collectingDays: (covered: number, days: number) => `collecting · ${covered}/${days} days`,
    collectedRange: (from: string, through: string) => `History collected ${from} – ${through} (UTC).`,
    collectionOff: "Historical collection isn't running on this server, so only the last-24h snapshot is available.",
    ...
    labelAllPairs: "All pairs",
    labelHistory: "Collected history",
    methodologyHistory:
      "A background job stores every listed pair's completed daily candle for all ten exchanges — a one-year backfill, then the last few days again each day after 00:15 UTC so late corrections are picked up. A day counts toward a period only once (almost) every pair on that exchange was fetched; until then the period shows how many days are in. Pairs are discovered from each exchange's current listing, so pairs delisted before collection started aren't in the backfill; once collected, a pair's history is kept even if it's later delisted.",
    methodologyAllPairs:
  ...
  flows: {
    ...
    noLabeledActivity: "This exchange's labeled wallets show no regular two-way activity (only a few transfers, or money only coming in or only going out) — its labels are likely out of date, so no figure is shown.",
    ...
    methodology: [
      [
        "What is counted",
        "USDT, USDC and native ETH on Ethereum (all ten exchanges), USDT on Tron (the six exchanges Dune labels there: Binance, OKX, Bybit, KuCoin, Gate, Bitget) and BTC on Bitcoin (Binance, OKX) moving into and out of each exchange's known wallets. Net flow = inflow − outflow, in the asset's own units; USDT and USDC are USD-pegged.",
      ],
      [
        "Which wallets",
        "Dune's exchange address labels (cex.addresses), plus the reserve wallets Binance (audit of 1 Sept 2026) and OKX (snapshot of 11 Aug 2026) publish in their proof of reserves. Checked against those lists, Dune's labels alone covered Binance's BTC 93%, USDT 95%, USDC 57%, ETH 94%, and OKX's BTC 54%, USDT 58%, USDC under 1%, ETH 18% of the reserve balance — on Tron, 85% of Binance's USDT and none of OKX's; with the published wallets added, those reserve wallets are effectively fully covered. For OKX this mostly moved transfers between its own hot and cold wallets out of inflow/outflow. Every other exchange uses Dune's labels alone and is marked \"unverified\": Coinbase, Upbit and Kraken publish no wallet list to check against, and Bybit shows its current list only to logged-in users. Mining-pool wallets and staking validators are left out.",
      ],
      [
        "ETH",
        "ETH counts every transfer of native ether, including ether moved by smart-contract calls. ETH an exchange deposits into Ethereum staking leaves its known wallets and so counts as outflow; wrapped ETH (WETH) is not included.",
      ],
      [
        "Bitcoin",
        "Bitcoin moves in transactions that spend whole earlier outputs and send change back. A transaction counts as an inflow for an exchange when that exchange isn't among its senders; when it is a sender, what goes to other addresses is outflow and what comes back to it (change, moves between its own wallets) is excluded.",
      ],
      [
        "Own wallets",
  ...
  volumeHistory: {
    ...
    rangeNote: (days: number) => `Last ${days} completed UTC days, from the collected history.`,
    loading: "Loading collected history…",
    error: "Could not load the volume history.",
    notCollected: "No collected history yet — the chart appears once the background collection has run for these exchanges.",
    groupingLabels: { day: "Daily", week: "Weekly", month: "Monthly" },
    metricLabels: { total: "All pairs", tracked: "Major pairs" },
    modeLabels: { usd: "USD", share: "Share %" },
    ariaGrouping: "Group bars by",
    ariaMetric: "Pairs included",
    ariaMode: "Show values as",
    ariaChart: (n: number) => `Stacked bar chart of exchange spot volume, ${n} bars. The table above shows the same figures.`,
    other: (names: string) => `Other (${names})`,
    otherShort: "Other",
    total: "Total",
    incompleteBucket: "Incomplete: not every day is in yet",
    note: "Weeks run Monday–Sunday, months are calendar months (UTC).",
    incompleteNote: "Faded bars are incomplete — a partial week or month at the edge of the range, or days not yet collected — and aren't comparable with full ones.",
    missingVenues: (names: string) => `Not collected yet: ${names}.`,
  },
  ...
  volumeComparisonSummary: {
    ...
    cexAllPairsPeriodRow: (n: number, window: string) => `CEX — ${n} exchange${n === 1 ? "" : "s"}, all spot pairs (${window} UTC)`,
